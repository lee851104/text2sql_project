...
| `dim_b_column` | `id` | 一列一個原始機組／彙總欄 | 64 個欄位全數保留。21 個沒有機組主檔的欄位改由 `configs/b_column_capacity.yaml` 指定的來源填 `capacity_kw`（核能主檔 6、8931 明細 9、8931 小計 3），`capacity_source` 記錄來源；8931 是快照，另附當次 SHA-256 前綴。麥寮 3 欄無可引用來源，維持 NULL |
...
| `dim_re_site` | `id` | 一列一再生能源發電站 | `re_sites.csv`；17141 同站的多個場址在入庫時彙總（容量與風機數相加、型號與申設狀態以 `|` 併列），場址明細只留在 CSV。`source` 區分官方主檔與 `re_sites_supplement.csv` 補上的站 |
| `fact_re_monthly` | `site_id, year, month` | 一站一月 | `re_generation.csv`；`generation_kwh` 是**淨發電量**（經台電簡明月報表 2-2 交叉驗證），官方標記無資料時存 NULL 而非 0，`value_status` 保留該格解析結果 |
...
| `v_peak` | `日期`、`機組欄位`、`尖峰出力_萬瓩`、`電廠`、`粒度`、`類別`、`對應裝置容量_萬瓩`、`容量來源`、殘差／彙總旗標 | 尖峰出力是瞬時功率；禁止跨日 `SUM` 解釋為發電量。容量對有機組主檔的欄位取自 crosswalk，其餘取自核能主檔或 8931，`容量來源` 可分辨；比值超過門檻的欄位會帶 `KNOWN_CAPACITY_GAP` 揭露 |
| `v_system` | `日期`、`尖峰負載_萬瓩`、`備轉容量_萬瓩`、`備轉容量率_pct`與用電指標 | 可做期間極值、日趨勢與同日比較 |
| `v_outage` | `機組名`、`電廠`、起訖日期、`原因`、`對齊狀態` | 未對齊列必須保留原始機組名，不得猜測主檔對應 |
| `v_re_generation` | `年度`、`月份`、`發電站`、`能源別`、`縣市`、`發電量_度`、`裝置容量_瓩`、`場址數`、`數值狀態`、`主檔來源` | **僅涵蓋台電自建場站**，任何查詢都會附 `RENEWABLE_SELF_BUILT_ONLY` 範圍揭露；月粒度電量可跨月加總，`數值狀態` 為 `missing` 的列是 NULL 不是 0 |
