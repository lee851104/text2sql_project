...
你打一句中文，系統把它翻成資料庫查詢語言（SQL），查完台電的公開資料，再用文字、數字和圖表回答你——不必先去找一位會寫 SQL 的人。

和一般 AI 問答不同的地方是：**它會先判斷這個問題資料答不答得出來。**答不出來就說答不出來，順便告訴你該怎麼問才對 —— 而不是生一個看起來很像答案的數字。

---

## 1. 解決什麼問題

**會問問題的人不會查資料，會查資料的人不知道要問什麼。**

想知道答案的是業務端。他們每天在想的是「這個月尖峰負載為什麼特別高」「哪幾台機組的歲修排在一起」「去年這個時候備轉容量有這麼低嗎」——問題很具體，但他們不會寫 SQL，也不知道資料表長什麼樣子。

會寫 SQL 的是資訊端。但他們不知道業務今天在煩什麼，也不見得清楚在這份資料裡「尖峰出力」和「發電量」是兩件完全不同的事。

於是實際流程長這樣：

```text
業務寫信描述需求  →  資訊猜他要什麼  →  寫查詢  →  回一張表
                                                      ↓
                    「不是這個意思…」  ←───────────────┘
```

一個五分鐘就能回答的問題，來回三天。而且每多一輪，業務就更不想問下一個問題——**真正的損失不是等待時間，是那些因為太麻煩而根本沒被問出口的問題。**

這個系統把中間那一段拿掉：業務直接用自己的話問，系統負責翻成 SQL。

### 但中間那個人不只是翻譯

這是這個專題真正花力氣的地方。

那位資訊人員（或熟悉資料的分析師）除了把需求翻成 SQL，還默默做了另一件事：**在需求不合理的時候喊停。**

> 業務問：**台中電廠去年總發電量是多少？**

把每天的數字加起來，確實會得到一個數字。但這個數字有兩個問題：

1. **單位就不對。** 台電這份資料記的是「每天用電最高的那一瞬間，機組出了多少力」，單位是萬瓩（功率），不是整天發了多少度電（電量）。把不同天的瞬間功率加起來，在物理上沒有意義——就像把每天中午十二點的車速加起來，當成一個月開了多少公里。
2. **資料本來就不完整。** 台中電廠有部分氣渦輪機組被收在全系統的合計欄位裡，從現有資料拆不回台中電廠自己的總量。

懂這份資料的人會當場說「這個不能這樣算」。但**一個只會翻譯的 Text2SQL 不會**——它語法正確、SQL 跑得動、畫面上不會有任何異狀，業務就拿著那個數字去開會了。

**把人拿掉的同時，也把那個會喊停的人一起拿掉了。**所以系統必須自己補上這個角色：拒絕這個計算、說明原因，並給出資料真的答得出來的問法。

> 「比較 2026 年各月尖峰日的台中燃煤機組出力」
> 「查詢 2026 年 7 月全系統尖峰負載最高的日期」

翻譯只是一半。另一半是**代替那位被拿掉的人說「這題不能這樣問，但你可以這樣問」**——這也是為什麼這個專題大部分的設計都在守門層，而不是在產生 SQL 那一段。

---

## 2. 實際操作

<!-- TODO: 補上操作示範動畫（查詢 → 守門攔截 → 改問法 → 圖表結果） -->
...
| **雙層守門** | 一層擋不安全的 SQL，一層擋「跑得動但答案是錯的」查詢——後者就是那位被拿掉的人原本在做的事 |
...
| 已識別並寫進資料庫的資料陷阱 | 10 類 |
| 離線評測 | 意圖 80／80、執行 60／60（語料內外各 30）、語意陷阱 45／45 |
| 守門誤攔 | 0／20（合法問題沒有被錯誤拒絕） |
| SQL 攻擊題庫 | 15／15 全數攔截 |
| 測試 | 397 項，分布在 38 個測試檔，CI 全程離線不需 API key |
