...
**再一次：CP-062 併進 main 之後，連「挑哪個範圍」這件事也不必自己做了。** CP-062 給 `check_question`
的是 `coverage_range` —— 全域宣告與所有檢視的聯集，理由寫在它的 docstring 裡：這一關跑在 route 之前，
不知道最後會查哪個檢視，唯一誠實說得出口的是「所有資料都涵蓋不到」，比這更窄就是在猜。既然 `v_outage`
已經是 `VIEW_TIME_SPANS` 的一員，聯集自動含到 2028-06-23，大修問句本來就過得了那一關，本分支那段
「用 `classify_intent` 判斷要拿哪個範圍比」因此整段刪掉，`classify_intent` 的 import 也跟著不需要了。

擋還是擋得住，只是換一關：實測「2027年3月台中#1的尖峰出力」在 `check_question` 是 OK，到
`check_sql` 看到 `FROM v_peak` 才擋下，訊息還更準 ——「這個查詢用到的檢視沒有問句那段期間的資料
（v_peak 涵蓋 2025-01-01 至 2026-07-31）」，指得出是哪個檢視。同一份設定下「2027年3月有哪些機組大修」
兩關都通過。原本釘住「問句層就擋」的那個測試改成釘這個分工。

順帶修掉測試替身的一個漏洞：`_guard_with_outage_range()` 的 `view_spans` 只放了 `v_outage`，
少了 `v_peak`。真實的 guard 是逐檢視量出來的，少一個就讓 `check_sql` 那一關找不到界線、不會擋，
測出來的行為比實際寬鬆。補上 `v_peak` 之後上面那一題才真的驗得到。
