...
### 已知落差：ruff 掃描範圍比 CI 大

CI 是乾淨簽出，只看得到受版控的檔案。本機跑 `ruff check .` 則會掃到未被 gitignore 的暫存目錄
（實測曾因 `extensions/` 產生 28 個與分支無關的錯誤，造成假的 BLOCK）。

判定 ruff 失敗時，先確認錯誤是不是落在這個分支真正改動的檔案上。這是待修項目。
